...
/* Global variables.  See socket.h for comments. */
char inet_thisMachineName[MAXHOSTNAMELEN];
char inet_thisMachineDots[20];
IP32bit inet_thisMachineIP;
char inet_farMachineDots[20];
IP32bit inet_farMachineIP;
short inet_farMachinePort;
...
int tcp_init(void)
{
	...
	rc = gethostname(inet_thisMachineName, MAXHOSTNAMELEN);
	if (rc)
		return -1;

/* convert from name to IP information */
	inet_thisMachineIP = inet_name_ip(inet_thisMachineName);
	if (inet_thisMachineIP == -1)
		return -1;
	strcpy(inet_thisMachineDots, inet_ip_dots(inet_thisMachineIP));
...
{
	IP32bit ip = inet_name_ip(name);
	if (ip == NULL_IP)
		return 0;
	return inet_ip_dots(ip);
}				/* inet_name_dots */

char *inet_ip_dots(IP32bit ip)
{
...
IP32bit inet_dots_ip(const char *s)
{
	struct in_addr a;
...
char *inet_dots_name(const char *s)
{
	return inet_ip_name(inet_dots_ip(s));
}				/* inet_dots_name */
...
int tcp_connect(IP32bit ip, int portnum, int timeout)
{
...
retry:
	...
	inet_farMachineIP = sa.sin_addr.s_addr;
	strcpy(inet_farMachineDots, inet_ip_dots(inet_farMachineIP));
	inet_farMachinePort = ntohs(sa.sin_port);
...
int tcp_read(int fh, char *buf, int buflen)
{
...
int tcp_readFully(int fh, char *buf, int buflen)
{
	int n = 0;
	int rc;
	while (buflen) {
...
int tcp_write(int fh, const char *buf, int buflen)
{
	int savelen = buflen;
	int n;
	while (buflen) {
...
Actually edbrowse doesn't need it any more, so comment it out.
*********************************************************************/
...
int main(int argc, char **argv)
{
	...
	if (inet_name_ip(inet_ip_name(ip)) != ip)
		puts("fail 1");
	if (inet_dots_ip(inet_ip_dots(ip)) != ip)
		puts("fail2");
	if (strcmp(inet_thisMachineDots,
		   inet_name_dots(inet_dots_name(inet_thisMachineDots))))
		puts("fail3");

	if (argc == 1 || argv[1][1] || !strchr("cl", argv[1][0])) {
		fprintf(stderr,
			"usage:  tcptest l portnum   # listen on a port\n\
